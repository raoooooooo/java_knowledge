...
```mermaid
graph LR
    subgraph session["Session 模式：共享常驻集群，main() 在客户端"]
        s_client["Client<br/>（本地/网关, 需下载依赖）"]
        s_cluster["常驻集群<br/>JM + TM（共享）<br/>Job1 / Job2 / Job3<br/>多个 job 挤一个集群, 互相干扰"]
        s_client -- "main() 生成 JobGraph" --> s_cluster
    end
    subgraph perjob["Per-Job 模式：每 job 一个集群，main() 仍在客户端（⚠️ 1.15 起弃用）"]
        p_client["Client<br/>（本地/网关, 仍需下载依赖）"]
        p_cluster["集群A（JM + TM）<br/>Job1<br/>一 job 一集群, 启停开销大"]
        p_client -- "main() 生成 JobGraph" --> p_cluster
    end
    subgraph app["Application 模式：每应用一个集群，main() 在集群 JM 内执行 ★ Flink 主推"]
        a_client["Client<br/>（轻量, 无需下载依赖）"]
        subgraph app_cluster["集群（JM 内跑 main()）"]
            a_main["main() 在 JM 内（核心区别）<br/>execute() -> Job1<br/>execute() -> Job2<br/>一个应用可含多个 job"]
            a_tm["TM(s)"]
        end
        a_client -- "只 ship jar + 参数" --> app_cluster
    end
```
