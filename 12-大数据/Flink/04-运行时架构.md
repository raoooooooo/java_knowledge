...
```
【窄依赖 Narrow】上游每实例 -> 只发给下游一个实例，无网络交换（走本地内存）

   上游算子            下游算子
   ┌────┐  ─────────►  ┌────┐
   │ A1 │   forward    │ B1 │      特征: 点对点 1->1
   └────┘              └────┘      · 同 TaskManager 内走本地内存
   ┌────┐  ─────────►  ┌────┐      · 不产生网络边界
   │ A2 │              │ B2 │      · 数据不重分布
   └────┘              └────┘      典型: map / filter / union / forward

【宽依赖 Wide】上游每实例 -> 发给下游多个实例，需网络 shuffle

   上游算子            下游算子
   ┌────┐    ╱────►  ┌────┐
   │ A1 │   ╱ ╲     │ B1 │      特征: 一对多 1->N
   └────┘  ╱   ╲    └────┘      · 跨网络传输 (shuffle)
   ┌────┐ ─     ────┌────┐      · 产生网络边界 (Network Partition)
   │ A2 │   ╲ ╱     │ B2 │      · 数据重分布
   └────┘    ╲      └────┘      典型: keyBy(hash) / rebalance / broadcast
                                   / shuffle / global
```
...
```
   ┌──────────┐ barrier1  ┌──────────┐
   │ 输入1     │ ────────► │  Op A   │  ← 多输入通道: 等所有 barrier 都到才做快照
   └──────────┘           │(fan-in>1)│    期间需缓存先到 barrier 之后的数据, 反压严重时甚至 OOM
   ┌──────────┐ barrier2  │          │
   │ 输入2     │ ────────► └──────────┘
   └──────────┘
```
