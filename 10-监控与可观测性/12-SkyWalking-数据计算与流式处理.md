...
│  │                         │                                 │  │
│  │  ┌────────────────────────────────────────────────────┐  │  │
│  │  │ ② OAL 引擎：指标聚合（Observability Analysis Lang） │  │  │
│  │  │     ├── 分钟级聚合（L1 Metrics）                     │  │  │
│  │  │     ├── 小时级聚合（L2 Metrics）                     │  │  │
│  │  │     └── 天级聚合（L3 Metrics）                       │  │  │
│  │  └────────────────────────────────────────────────────┘  │  │
...
**三级降采样**：分钟级（L1）→ 小时级（L2）→ 天级（L3）

**为什么需要**：
1. **存储成本**：原始粒度数据存储成本极高，降采样可大幅减少存储空间
2. **查询性能**：查询长时间范围的数据时，降采样后的数据量更小，查询更快
3. **数据生命周期**：不同粒度的数据有不同的保留策略（分钟级 7 天、小时级 30 天、天级 365 天）
