...
```mermaid
graph TB
    subgraph case1["方案一：大量 key 同时过期"]
        A1[T0: 1万个key同时过期] --> A2[1万请求同时 miss] --> A3[(DB)]
    end
    subgraph case2["方案二：Redis 宕机"]
...
```mermaid
graph TB
    subgraph hotkey_replica["热 key 多副本"]
        direction TB
...
```mermaid
graph TB
    subgraph original["原系统"]
        Original[原 Topic 4分区] --> Con4[消费者 4个<br/>处理慢]
    end
    subgraph expand["应急扩容"]
        Original --> Forward[转发]
...
```mermaid
graph LR
    subgraph delay_levels["RocketMQ 延迟等级（共 18 级）"]
        L1["1: 1s"]
        L2["2: 5s"]
        L3["3: 10s"]
        L4["4: 30s"]
