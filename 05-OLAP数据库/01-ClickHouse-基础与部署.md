...
│                                                                 │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐        │
│  │ CH-01-01 │  │ CH-02-01 │  │ CH-03-01 │  │ CH-04-01 │  数据节点  │
│  │ CH-01-02 │  │ CH-02-02 │  │ CH-03-02 │  │ CH-04-02 │  (每分片2副本) │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘        │
│       Shard 1       Shard 2       Shard 3       Shard 4         │
│       (5节点)       (5节点)      (5节点)       (5节点)         │
│                                                                 │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐                      │
│  │  ZK-01   │  │  ZK-02   │  │  ZK-03   │    ZooKeeper 集群    │
│  │ (独立部署)│  │ (独立部署)│  │ (独立部署)│    (3节点)         │
│  └──────────┘  └──────────┘  └──────────┘                      │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
...
客户端 INSERT
    ↓
1. 原始表（MergeTree）← ✅ 存完整明细数据（100%）
    raw_api_logs
    ↓ 写入成功后自动触发
2. 物化视图（MV）← ❌ 本身不存数据！只是「触发器 + SQL逻辑」
    api_stat_mv
    执行 SELECT + GROUP BY + State 函数转换
    ↓ 计算结果写入
3. 聚合表（AggregatingMergeTree）← ✅ 存预聚合结果（1%~10%）
    api_stat_hourly
```
