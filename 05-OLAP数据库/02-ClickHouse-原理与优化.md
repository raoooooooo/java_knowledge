...
```
分片1
    ├── 副本1（CH-01-01）← 主副本（负责写入）
    ├── 副本2（CH-01-02）← 从副本（异步同步
    ├── 副本3（CH-01-03）← 从副本
    ├── 副本4（CH-01-04）← 从副本
    └── 副本5（CH-01-05）← 从副本
```
...
2. 副本1写入本地磁盘成功
      ↓
3. 副本1向 ZK 写入一个「日志条目（Log Entry）
   ├── 包含：块编号、数据校验和、大小
   └── 本质是："我这里有新数据了，编号是12345"
      ↓
4. 其他副本（2-5）通过 ZK Watch 机制发现有新数据
      ↓
5. 副本2-5分别向副本1发起「拉取数据」请求
      ↓
6. 数据点对点传输（不是通过 ZK 传！）
      ↓
7. 各副本写入成功后，在 ZK 上标记完成
```
...
```
/data/clickhouse/data/db_name/table_name/
    ├── 20240101_20240101_1_1_0/  ← 一个Part目录
    │   ├── checksums.txt                  ← 校验和
    │   ├── columns.txt                    ← 列信息
    │   ├── count.txt                      ← 行数
    │   ├── primary.idx                    ← 主键索引（最重要！）
    │   ├── timestamp.mrk2                 ← 标记文件（偏移量）
    │   ├── timestamp.bin                  ← timestamp列的数据
    │   ├── level.mrk2
    │   ├── level.bin
    │   └── message.bin
    └── ...
...
WHERE timestamp >= '2024-01-01' 
  AND hostname = 'host001'
      ↓
┌─────────────────────────────────┐
│ 1. 分析 WHERE 条件，定位分区   │
│    └── 只扫描 20240101 分区  │  ← 分区裁剪，跳过其他分区
└─────────────────────────────────┘
      ↓
┌─────────────────────────────────┐
│ 2. 用主键索引快速定位        │
│    └── 查 primary.idx          │  ← 二分查找，O(logN)
└─────────────────────────────────┘
      ↓
┌─────────────────────────────────┐
│ 3. 读取标记文件 .mrk         │
...
└─────────────────────────────────┘
      ↓
┌─────────────────────────────────┐
│ 4. 只读取需要的列文件        │  ← 列式存储，不用的列完全不读
└─────────────────────────────────┘
      ↓
┌─────────────────────────────────┐
│ 5. 向量化计算结果            │  ← SIMD批量计算，速度飞起
└─────────────────────────────────┘
```
...
└─────────────────────────────────┘
      ↓ 分发到4个分片
┌──────┬──────┬──────┬──────┐
│ 分片1  │ 分片2  │ 分片3  │ 分片4  │  ← 并行查，4个分片同时查
└──────┴──────┴──────┴──────┘
      ↓ 每个分片返回中间结果
┌─────────────────────────────────┐
│ 2. 协调节点聚合所有中间结果      │  ← Merge中间结果合并
│    COUNT 直接相加
│    GROUP BY 二次聚合
│    ORDER BY 二次排序
└─────────────────────────────────┘
...
1. 写入原表成功
      ↓
2. 触发 MV 计算（对这批数据执行 SELECT + GROUP BY）
      ↓
3. 计算结果写入 SummingMergeTree
```
