...
```mermaid
graph TB
    agent_pre["Agent 端预处理（不叫 L1，只是 Segment 组装）<br/>Span → Segment → 过滤 → 序列化 → 上报"]

    subgraph oap_agg["OAP 端聚合（L1/L2/L3 三级降采样都在这里）"]
        step1["① TraceAnalyzer：解析 Segment，提取 Source 数据<br/>Entry Span → Service/Endpoint 指标<br/>Exit Span → Relation 指标 + DB/MQ 指标<br/>Local Span → 附加分析"]
        step2["② OAL 引擎：实时聚合（每来一条数据更新指标）<br/>产出 L1 指标（分钟级，1 分钟一个值）"]
        step3["③ 定时降采样任务（每小时/每天触发）<br/>每小时：60 个 L1 → 1 个 L2（小时级）<br/>每天：24 个 L2 → 1 个 L3（天级）"]
        step4["④ TopN 计算：滑动窗口 + 最小堆<br/>⑤ 慢查询检测：阈值判定 + 采样<br/>⑥ 热力图：慢端点分布统计"]
...
```mermaid
graph TD
    oal_file["OAL 脚本（.oal 文件）"]

    oal_file --> compile["1. 编译阶段"]
    compile --> c1["OALParser（ANTLR4 解析器）解析语法"]
    compile --> c2["生成 Java 源代码（OALClassGenerator）"]
    compile --> c3["编译生成 .class 文件"]

    oal_file --> load["2. 加载阶段"]
    load --> l1["加载编译后的 .class 文件"]
    load --> l2["反射创建指标聚合实例"]
    load --> l3["注册到 MetricsStreamProcessor"]

    oal_file --> exec["3. 执行阶段"]
    exec --> e1["接收 Source 数据（来自 TraceAnalyzer）"]
    exec --> e2["调用聚合实例的 combine() 方法"]
    exec --> e3["分钟级聚合：每 1 分钟计算一次"]
    exec --> e4["小时级聚合：每 1 小时计算一次"]
    exec --> e5["天级聚合：每 24 小时计算一次"]
```
...
```mermaid
graph LR
    s1["1. 请求到达<br/>创建 Entry Span"] --> s2["2. 调用下游<br/>创建 Exit Span"] --> s3["3. 本地操作<br/>创建 Local Span"] --> s4["4. 请求结束<br/>所有 Span 完成"] --> s5["5. 组装 Segment<br/>包含所有 Span 和一个 segmentId"] --> s6["6. 序列化 Segment<br/>Protobuf 编码"] --> s7["7. 放入 TraceBuffer<br/>等待批量上报"] --> s8["8. 批量上报<br/>gRPC 流式发送到 OAP"]
```
...
graph TB
    raw["原始数据（Segment）"]
    raw --> l1["L1 聚合（分钟级）<br/>窗口大小：1 分钟<br/>聚合方式：实时聚合（每来一条数据就更新）<br/>存储周期：7 天（默认）<br/>示例：service_cpm = 每分钟请求数"]
    l1 --> l2["L2 聚合（小时级）<br/>窗口大小：1 小时<br/>聚合方式：从 L1 数据二次聚合<br/>存储周期：30 天（默认）<br/>示例：service_cpm_hour = 每小时平均请求数"]
    l2 --> l3["L3 聚合（天级）<br/>窗口大小：1 天<br/>聚合方式：从 L2 数据二次聚合<br/>存储周期：365 天（默认）<br/>示例：service_cpm_day = 每天平均请求数"]
```
...
```mermaid
graph TB
    raw["原始数据（Segment，每条请求一份）"]
    raw -- "OAP 实时聚合（每来一条更新指标）" --> l1["L1：分钟级聚合（最细粒度）<br/>时间窗口：1 分钟<br/>产出：每分钟一个指标值<br/>存储周期：7 天（默认）<br/>用途：看「最近」的实时监控"]
    l1 -- "每小时定时任务，把 60 个 L1 值聚合成 1 个" --> l2["L2：小时级聚合（中等粒度）<br/>时间窗口：1 小时<br/>产出：每小时一个指标值<br/>存储周期：30 天（默认）<br/>用途：看「最近一个月」的趋势"]
    l2 -- "每天定时任务，把 24 个 L2 值聚合成 1 个" --> l3["L3：天级聚合（最粗粒度）<br/>时间窗口：1 天<br/>产出：每天一个指标值<br/>存储周期：365 天（默认）<br/>用途：看「一年」的长期趋势"]
```
...
```mermaid
xychart-beta
    title "Endpoint 热力图（延迟分布，P99）"
    x-axis ["端点 A", "端点 B", "端点 C"]
    y-axis "P99 延迟（ms）" 0 --> 500
    bar [50, 100, 500]
```

热力图按延迟区间统计请求分布：每个端点一行，按 `0-50ms`、`50ms+` 等桶统计请求占比，P99 越高的端点越"热"，一眼就能看出慢端点集中在哪里。
