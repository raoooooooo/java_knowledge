...
```mermaid
graph LR
    subgraph init["初始状态（堆内存100MB）"]
        direction TB
        Eden0["Eden：80MB<br/>对象优先分配在这里"]
        S00["S0：10MB（空的）"]
        S10["S1：10MB（空的）"]
    end
    subgraph gc1["第一次GC"]
        direction TB
        G1A["Eden 中存活对象 + S0 中存活对象<br/>全部复制到 S1"]
        G1B["清空 Eden 和 S0"]
        G1A --> G1B
    end
    subgraph gc2["第二次GC"]
        direction TB
        G2A["Eden 中存活对象 + S1 中存活对象<br/>全部复制到 S0"]
        G2B["清空 Eden 和 S1"]
        G2A --> G2B
    end
    init --> gc1
    gc1 --> gc2
    gc2 -.->|"循环往复（S0/S1 角色互换）"| gc1
```
...
```mermaid
graph TD
    A["并发标记完了，开始并发清除"] --> B["用户线程又产生了新的垃圾<br/>（比如把某个引用设为 null）"]
    B --> C["这些垃圾是标记完之后才产生的<br/>这次 GC 清理不到"]
    C --> D["它们叫「浮动垃圾」<br/>只能等下一次 GC 再清理"]
```
